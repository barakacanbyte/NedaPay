      ...
      if (merchantAddress) {
        try {
          ...
          if (response.ok) {
            ...
            setRecentLinks(links.map((link: any) => ({
              id: link.id,
              createdAt: new Date(link.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }),
              amount: link.amount,
  ...
  const handleCreateLink = async (e: React.MouseEvent | React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRecentLinks(prev => [{
          id: newLink.id,
          createdAt: new Date(newLink.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }),
                ...
                <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                  {recentLinks.length === 0 ? (
                    ...
                    recentLinks.map((link) => (
                      <tr key={link.id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">{link.createdAt}</td>
