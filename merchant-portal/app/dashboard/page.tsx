...
const fetchTransactionsFromDB = async (
  ...
  try {
    const response = await fetch(`/api/transactions?merchantId=${selectedWalletAddress}`);
    if (!response.ok) {
...
export default function MerchantDashboard() {
  ...
  useEffect(() => {
    if (isConnected && selectedWalletAddress) {
      fetchTransactionsFromDB(selectedWalletAddress, setTransactions, setIsTransactionLoading);
    }
          ...
          const onTransfer = async (from: string, to: string, value: ethers.BigNumber, event: any) => {
            ...
            if (to.toLowerCase() === selectedWalletAddress.toLowerCase()) {
              ...
              if (!dbHashes.has(txHash)) {
                ...
                toast.success(`Payment received: ${parseFloat(ethersLib.utils.formatUnits(value, decimals))} ${symbol} from ${shortSender}`);
                fetchTransactionsFromDB(selectedWalletAddress, setTransactions, setIsTransactionLoading);
              }
            }
          };
      ...
      for (const coin of filteredCoins) {
        try {
          ...
          try {
            ...
            console.warn(`Could not fetch balance/decimals for ${coin.baseToken}:`, tokenErr?.message);
                    ...
                    options={{
                      ...
                      scales: {
                        y: {
                          ...
                          ticks: {
                            color: typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? '#9ca3af' : '#4b5563',
                          },
                        },
                        x: {
                          grid: { display: false },
                      ...
                      plugins: {
                        legend: {
                          labels: {
                            color: typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? '#fff' : '#222',
                            usePointStyle: false,
